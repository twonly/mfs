...
uint8_t* matoclserv_createpacket(matoclserventry *eptr,uint32_t type,uint32_t size) { // the packet sent back to client, appended to the queue
	...
	put32bit(&ptr,type); //4byte
	put32bit(&ptr,size); //4byte
	outpacket->startptr = (uint8_t*)(outpacket->packet);
	outpacket->next = NULL;
	*(eptr->outputtail) = outpacket; //eptr is the packet list
...
//	uint16_t port;
	...
	auid=0;
	agid=0;
	for (eaptr = matoclservhead ; eaptr && eptr==NULL ; eaptr=eaptr->next) { //once the eptr is not NULL, break
		if (eaptr->mode!=KILL) {
			acl = &(eaptr->chunkdelayedops);
			while (*acl && eptr==NULL) {
				cl = *acl;
				if (cl->chunkid==chunkid) {
					eptr = eaptr; //got it and set the value
					qid = cl->qid;
					fleng = cl->fleng;
					type = cl->type;
					inode = cl->inode;
	...
	case FUSE_WRITE:
		...
		memcpy(ptr,loc,count*6); //location - ip + port
//		for (i=0 ; i<count ; i++) {
...
//	if (FD_ISSET(lsock,rset)) {
		ns=tcpaccept(lsock);  //lsock is a static variable, ns is the serve socket for client
		if (ns<0) {
			...
			matoclservhead = eptr;
			eptr->sock = ns; //set the sock  yujy
			eptr->pdescpos = -1;
...
// write
	for (eptr=matoclservhead ; eptr ; eptr=eptr->next) { //write to each entry
