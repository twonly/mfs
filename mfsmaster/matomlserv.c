...
void matomlserv_read(matomlserventry *eptr) {
	...
	for (;;) {
		...
		if (eptr->mode==DATA) {
			...
			matomlserv_gotpacket(eptr,type,eptr->inputpacket.packet,size); //deal with different type of commands
...
void matomlserv_write(matomlserventry *eptr) { //from master to metalogger
	packetstruct *pack;
	int32_t i;
	for (;;) {
...
void matomlserv_serve(struct pollfd *pdesc) {
	...
	if (lsockpdescpos>=0 && (pdesc[lsockpdescpos].revents & POLLIN)) {
		...
		if (ns<0) {
			...
			passert(eptr);
			eptr->next = matomlservhead; //add a matomlserventry
			matomlservhead = eptr;
			eptr->sock = ns;
			eptr->pdescpos = -1;
			eptr->mode = HEADER;
