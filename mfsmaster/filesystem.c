...
#ifdef EDGEHASH
	if (node->data.ddata.elements>LOOKUPNOHASHLIMIT) { //LOOKUPNOHASHLIMIT? To improve the efficiency, use the hash table only when the elements are more than a pre-set arg
		ei = edgehash[EDGEHASHPOS(fsnodes_hash(node->id,nleng,name))];
		while (ei) {
			if (ei->parent==node && nleng==ei->nleng && memcmp((char*)(ei->name),(char*)name,nleng)==0) {
				return ei;
...
#else
	ei = node->data.ddata.children; //children in this directory
	while (ei) {
		if (nleng==ei->nleng && memcmp((char*)(ei->name),(char*)name,nleng)==0) {
			return ei;
		}
...
*/
	...
	parent->data.ddata.children = e; //add e as parent.children in the first place
	e->prevchild = &(parent->data.ddata.children); //e->prevchild = &e;
    //parent
	e->nextparent = child->parents;
	if (e->nextparent) {
		e->nextparent->prevparent = &(e->nextparent); //the prevparent points to itself
	}
	child->parents = e;
	e->prevparent = &(child->parents); //why always pointing to itself
#ifdef EDGEHASH
...
*/
	...
	p->parents = NULL;
    //put these into the fsnodes_link
...
//	node->mtime = node->ctime = ts;
	...
	nodehash[nodepos] = p;
	fsnodes_link(ts,node,p,nleng,name); //node is parent, p is child
	return p;
}
...
uint8_t fs_lookup(uint32_t rootinode,uint8_t sesflags,uint32_t parent,uint16_t nleng,const uint8_t *name,uint32_t uid,uint32_t gid,uint32_t auid,uint32_t agid,uint32_t *inode,uint8_t attr[35]) {
	fsnode *wd,*rn; //rn means rootnode? What about wd? Working directory (pwd)
	fsedge *e;
/*
#ifdef CACHENOTIFY
	...
	*inode = 0;
	...
	if (rootinode==MFS_ROOT_ID) { //from session
		...
		if (parent==MFS_ROOT_ID) {
			...
			if (!fsnodes_isancestor(rn,wd)) { //whethe rn is ancestor of wd
				return ERROR_EPERM;
			}
		}
	}
	...
	if (fsnodes_namecheck(nleng,name)<0) { //check the format of name
		return ERROR_EINVAL;
	}
...
uint8_t fs_mknod(uint32_t rootinode,uint8_t sesflags,uint32_t parent,uint16_t nleng,const uint8_t *name,uint8_t type,uint16_t mode,uint32_t uid,uint32_t gid,uint32_t auid,uint32_t agid,uint32_t rdev,uint32_t *inode,uint8_t attr[35]) {
	...
	*inode = 0;
	...
	if (fsnodes_nameisused(wd,nleng,name)) { //like lookup but only return true or false instead of NULL or a fsedge
		return ERROR_EEXIST;
	}
	if (fsnodes_test_quota(wd)) { //exceed the quota
		return ERROR_QUOTA;
	}
...
int fs_loadedge(FILE *fd,int ignoreflag) { //toread
